...
public class ApprovedOrders {
    ...
    public ApprovedOrders(){
        try{
            ...
            getOrders = connection.prepareStatement("SELECT ? FROM approved_orders;");

            deleteOrders = connection.prepareStatement("DELETE FROM approved_orders"
                    + "WHERE order_number = ?;");
            insertOrders = connection.prepareStatement("INSERT INTO approved_orders"
                    + "VALUES (?, ?, ?, ?, ?, ?, ?, ?);");
     ...
     */
    public ResultSet getApprovedOrders(String columnsNeeded){
        try{
     ...
     */
    public int insertApprovedOrders(int farmer_id, int order_number, String description, String futures_contract, String quantity, int strike, double cost_per_ton, Timestamp timestamp){
        int result=0;
        try{
     ...
     * @return 1 if success, 0 if fail.
     */
    public int deleteApprovedOrders(int order_Number){
        int result = 0;
        try{
